      ...
      module_function
        ...
        if is_numeric?(dice)
          die = rand(1..dice.to_i)
            "#{event.user.name} rolls thier dice and rolls #{die}"
        end
        ...
        if (dice != '0') && (dice.to_i.to_s != dice.strip)
          ...
          if dice.empty?
            roll = rand(1..20)
            "#{event.user.name} throws their D20 down and rolls `#{roll}`"
          #input is #d#
          else
            ...
            "#{event.user.name} throws their dice down and rolls `#{roll.join(', ')} = #{total}`"
          end
        end
      end
    end
